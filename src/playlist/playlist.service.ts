...
export class PlaylistService {
    ...
    async delete(id: string, userId: string) {
        ...
        if (!playlist) {
            throw new NotFoundException('Playlist sa pa egziste');
        }

        // Sekirite: Tcheke si se moun ki te kreye l la k ap efase l
        if (playlist.userId !== userId) {
            throw new ForbiddenException('Ou pa gen dwa efase playlist sa a');
        }
    ...
    async update(id: string, userId: string, name: string) {
        ...
        if (playlist.userId !== userId) throw new ForbiddenException('Ou pa gen dwa modifye sa');
